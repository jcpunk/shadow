...
static bool range_exists(struct commonio_db *db, const char *owner)
{
	...
	while (NULL != (range = commonio_next(db))) {
		if (subid_owner_match(range->owner, owner))
			return true;
	}
	return false;
}
...
/*
 * subid_owner_match: test whether two owner fields refer to the same user.
 *
 ...
 * are resolved to uid_t via passwd and compared numerically.
 * This should account for overlaping UIDs.
 *
...
subid_owner_match(const char *a, const char *b)
{
	...
	if (strisdigit(b)) {
		...
		const struct passwd *pw = getpwnam(b);
		if (NULL == pw)
			return false;
		id_b = pw->pw_uid;
	}
...
bool new_subid_range(struct subordinate_range *range, enum subid_type id_type, bool reuse)
{
	...
	if (reuse) {
		while (NULL != (r = commonio_next(db))) {
			if (!subid_owner_match(r->owner, range->owner))
				continue;
